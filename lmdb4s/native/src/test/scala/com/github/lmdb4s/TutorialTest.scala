...
import com.github.lmdb4s.GetOp.MDB_SET
import com.github.lmdb4s.Lmdb.createEnv
import com.github.lmdb4s.SeekOp.{MDB_FIRST, MDB_LAST, MDB_PREV}
import com.github.lmdb4s.TestUtils._
...
object TutorialTest extends TestSuite {
  ...
  def tutorial1(): Unit = {
    ...
    val key = "greeting".getBytes(UTF_8)
    val value = "Hello world".getBytes(UTF_8)
    ...
    try {
      ...
      assert( new String(fetchedVal, UTF_8) == "Hello world")
    } finally {
  ...
  /*def tutorial2(): Unit = {
    ...
    //val key = allocateDirect(env.getMaxKeySize())
    //val value = allocateDirect(700)
    val key = "key1".getBytes(UTF_8)
    val value = "lmdb".getBytes(UTF_8)
    ...
    try {
      ...
      //value.put("lmdb".getBytes(UTF_8)).flip()
    ...
    es.execute(new Runnable {
      def run(): Unit = {
        ...
        try {
          ...
          db.put(txn, key, "key2".getBytes(UTF_8))
          txn.commit()
        } finally {
          if (txn != null) txn.close()
        }
  ...
  def tutorial3(): Unit = {
    ...
    try { // A cursor always belongs to a particular Dbi.
      ...
      //value.put("lmdb".getBytes(UTF_8)).flip()
      key = "zzz".getBytes(UTF_8)
      value = "lmdb".getBytes(UTF_8)

      c.put(key, value)
      //key.clear()
      //key.put("aaa".getBytes(UTF_8)).flip()
      key = "aaa".getBytes(UTF_8)
      c.put(key, value)
      //key.clear()
      //key.put("ccc".getBytes(UTF_8)).flip()
      key = "ccc".getBytes(UTF_8)
      c.put(key, value)
  ...
  def tutorial5(): Unit = {
    ...
    //val key = allocateDirect(env.getMaxKeySize())
    //val value = allocateDirect(env.getMaxKeySize())
    var key: Array[Byte] = null
    var value: Array[Byte] = null
    ...
    try {
      ...
      //value.put("xxx".getBytes(UTF_8)).flip()
      key = "key".getBytes(UTF_8)
      value = "xxx".getBytes(UTF_8)
      c.put("key".getBytes(UTF_8), value)
      //value.clear()

      //value.put("kkk".getBytes(UTF_8)).flip()
      value = "kkk".getBytes(UTF_8)
      c.put(key, value)
      //value.clear()

      //value.put("lll".getBytes(UTF_8)).flip()
      value = "lll".getBytes(UTF_8)
      c.put(key, value)
      ...
      assert(new String(c.`val`, UTF_8) == "kkk")

      c.seek(MDB_LAST)
      //assert(UTF_8.decode(c.`val`).toString == "xxx")
      assert(new String(c.`val`, UTF_8) == "xxx")

      c.seek(MDB_PREV)
      //assert(UTF_8.decode(c.`val`).toString == "lll")
      assert(new String(c.`val`, UTF_8) == "lll")
  ...
  /**
   * Next up we'll show you how to easily check your platform (operating system and Java version)
   * is working properly with LmdbJava and the embedded LMDB native library.
   *
   */
  //@SuppressWarnings(Array("ConvertToTryWithResources"))
  /*def tutorial6(): Unit = {
  ...
  def tutorial7(): Unit = {
    ...
    val value = new UnsafeBuffer(allocateDirect(700))
    try {
      val txn = env.txnWrite
      try {
        try {
          ...
          try { // Agrona is faster than ByteBuffer and its methods are nicer...
            value.putStringWithoutLengthUtf8(0, "The Value")
